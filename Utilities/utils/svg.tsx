...
function screenLayout(kind: number, color: string, accent: string): string {
  if (kind === 0) {
    return `
      ...
      <rect x="20" y="380" width="350" height="165" rx="18" fill="#141414" fill-opacity="0.7" />
  ...
  if (kind === 1) {
    return `
      <rect x="20" y="132" width="350" height="210" rx="18" fill="#141414" fill-opacity="0.75" />
      ...
      <rect x="36" y="422" width="334" height="42" rx="11" fill="#141414" fill-opacity="0.85" />
      <rect x="36" y="472" width="334" height="42" rx="11" fill="#141414" fill-opacity="0.85" />
    `
  }
  if (kind === 2) {
    return `
      ...
      <rect x="38" y="194" width="334" height="78" rx="12" fill="#141414" fill-opacity="0.78" />
      ...
      <rect x="20" y="312" width="170" height="230" rx="18" fill="#141414" fill-opacity="0.75" />
      <rect x="200" y="312" width="170" height="230" rx="18" fill="#141414" fill-opacity="0.75" />
  ...
  return `
    <rect x="20" y="132" width="350" height="410" rx="20" fill="#141414" fill-opacity="0.78" />
...
export function svgScreen(color: string, accent: string, label: string, index: number): string {
  const dots = [0, 1, 2, 3].map((i) =>
    `<circle cx="${174 + i * 14}" cy="784" r="3.5" fill="${i === index % 4 ? accent : '#ffffff'}" fill-opacity="${i === index % 4 ? 0.9 : 0.3}" />`
  ).join('')
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="390" height="844" viewBox="0 0 390 844">
    ...
    <rect x="20" y="26" width="350" height="38" rx="12" fill="#141414" fill-opacity="0.75" />
    <text x="40" y="49" font-size="12" font-family="Inter, Arial, sans-serif" fill="#ffffff">9:41</text>
    <rect x="286" y="39" width="62" height="12" rx="6" fill="#ffffff" fill-opacity="0.12" />

    <rect x="20" y="76" width="350" height="44" rx="14" fill="#141414" fill-opacity="0.82" />
    <text x="36" y="103" font-size="16" font-family="Inter, Arial, sans-serif" font-weight="700" fill="#ffffff">${label}</text>

    ${screenLayout(index % 4, color, accent)}

    <rect x="20" y="740" width="350" height="72" rx="18" fill="#141414" fill-opacity="0.82" />
